...
class TestExecute:
    ...
    def test_walk_override_drives_llm_config(self, populated_storage, monkeypatch, tmp_path):
        ...
        populated_storage.execute_insert(
            ...
            " VALUES (?, ?, ?, ?)",
            ("book-1", "voice_audition", "temperature", json.dumps(0.9)),
        )
        populated_storage.execute_insert(
            "INSERT INTO walk_override (book_id, walk_name, key, value_json)"
            " VALUES (?, ?, ?, ?)",
            ("book-1", "voice_audition", "model_name", json.dumps("gpt-4o-mini")),
        )
        ...
        monkeypatch.setattr(
            "app.pipeline.walks.walk_2g_voice_audition.chat_completion",
            mock_call_llm,
        )
    ...
    def test_prompt_override_drives_system_prompt(
        ...
        assert captured["system_prompt"] == (
            "You are a voice casting specialist for audiobook production, "
