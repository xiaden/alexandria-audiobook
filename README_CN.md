...
- **自然停顿** - 可配置的说话人切换暂停（默认 500 ms）与同说话人继续时的暂停（默认 250 ms），透传至渲染边界供合并步骤使用
...
| Speaker Change Pause | 说话人切换暂停（透传至渲染边界供合并步骤使用；默认 500 ms） |
| Same Speaker Pause | 同说话人继续时的暂停（透传至渲染边界供合并步骤使用；默认 250 ms） |
