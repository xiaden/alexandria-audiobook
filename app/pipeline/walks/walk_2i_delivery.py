...
def _call_llm(
    ...
    messages = [
        {
            "role": "system",
            "content": "You are a TTS delivery specialist for audiobook production, generating performance instructions for text-to-speech rendering.",
        },
...
def _parse_llm_response(response_text: str) -> dict:
    ...
    try:
        instruct_data = json.loads(response_text)
    except json.JSONDecodeError:
        # Try to find JSON object in response with regex fallback
        match = re.search(r"\{[\s\S]*\}", response_text)
        if match:
            try:
                instruct_data = json.loads(match.group(0))
            except json.JSONDecodeError:
                logger.error(
