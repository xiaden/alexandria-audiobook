...
app.mount(
    "/designed_voices",
    StaticFiles(directory=DESIGNED_VOICES_DIR),
    name="designed_voices",
)
...
app.mount(
    "/dataset_builder",
    StaticFiles(directory=DATASET_BUILDER_DIR),
...
app.add_exception_handler(
    ConcurrentTransactionError, concurrent_transaction_error_handler
)
                ...
                for card_data in data.values():
                    ...
                    for key in ("GPU use (%)", "GPU Use (%)", "GPU Activity"):
                        ...
                        if gpu_use_str is not None and gpu_use_str != "N/A":
                            ...
                            break
                    break
        except Exception:  # noqa: BLE001 — best-effort GPU stats; probing must never crash the stats endpoint
            stats["utilization_percent"] = None

    except Exception as e:  # noqa: BLE001 — best-effort GPU stats; unreadable probe degrades to None
...
def check_disk_space(path, required_gb):
    ...
    except Exception:  # noqa: BLE001 — best-effort disk check; any probe error defaults to 'has space'
        return True, 0
...
async def get_system_stats():
    ...
    has_space, free_gb = check_disk_space(
        ROOT_DIR, 1.0
    )  # 1GB threshold for generic warning

    return {
        "gpu": gpu,
        "disk": {"free_gb": round(free_gb, 2), "low_space": not has_space},
    }
...
class LLMConfig(BaseModel):
    ...
    api_key: str
    model_name: str
    reasoning_effort: str | None = None  # Global default reasoning effort
...
class TTSConfig(BaseModel):
    ...
    batch_seed: int | None = None  # Single seed for batch mode, None/-1 = random
    compile_codec: bool = (
        False  # torch.compile the codec for ~3-4x batch throughput (slow first run)
    )
    ...
    sub_batch_max_items: int = (
        0  # hard cap on sequences per sub-batch (0 = auto from VRAM estimate)
    )
    batch_group_by_type: bool = (
        False  # group chunks by voice type for efficient batching
    )
    pause_between_speakers_ms: int = (
        500  # silence (ms) between different speakers during merge
    )
    pause_same_speaker_ms: int = (
        250  # silence (ms) when same speaker continues during merge
    )
...
class LoraGenerateDatasetRequest(BaseModel):
    ...
    samples: list[LoraDatasetSample] | None = None  # emotion+text pairs
    texts: list[str] | None = None  # legacy: flat text list (no emotions)
    language: str | None = None


class DatasetSampleGenRequest(BaseModel):
    description: (
        str  # full voice description (root + emotion already combined by frontend)
    )
    text: str
...
class DatasetBatchGenRequest(BaseModel):
    ...
    indices: list[int] | None = None  # which rows to generate (None = all)
    global_seed: int = -1  # -1 = random, >= 0 = same seed for all lines
    seeds: list[int] | None = None  # per-line seeds (overrides global_seed)
...
def run_process(command: list[str], task_name: str):
    ...
    try:
        ...
        if state.get("cancel"):
            ...
            state["status"] = "failed"
            state["logs"].append(
                f"[ERROR] Task {task_name} failed with return code {return_code}."
            )

    except Exception as e:  # noqa: BLE001 — external task run; any failure marks the task failed
        logger.error(f"Error running {task_name}: {e}")
        state["status"] = "failed"
        state["logs"].append(f"[ERROR] Error: {e!s}")
    finally:
        state["process"] = None
        state["running"] = False
...
def _stream_subprocess_to_logs(
    ...
    reader = threading.Thread(
        target=_reader, args=(process.stdout, log_queue), daemon=True
    )
    reader.start()
...
async def save_config(request: Request, config: AppConfig):
    ...
    if os.path.exists(CONFIG_PATH):
        async with aiofiles.open(CONFIG_PATH, "r", encoding="utf-8") as f:
            current = json.loads(await f.read())
    else:
        current = {}
...
async def voice_design_preview(request: VoiceDesignPreviewRequest):
    ...
    try:
        wav_path, _sr = engine.generate_voice_design(
    ...
    except Exception as e:  # noqa: BLE001 — TTS engine failures surface as HTTP 500
...
async def clone_voices_upload(file: UploadFile = File(...)):
    ...
    if ext not in ALLOWED_AUDIO_EXTS:
        raise HTTPException(
            status_code=400,
            detail=f"Unsupported format. Use: {', '.join(ALLOWED_AUDIO_EXTS)}",
        )
...
def _safe_extract_zip(zf, destination):
    ...
    for member in zf.infolist():
        ...
        try:
            is_within_destination = (
                os.path.commonpath((destination, member_path)) == destination
            )
        except ValueError:
            is_within_destination = False
        if not is_within_destination:
            raise HTTPException(
                status_code=400, detail="ZIP contains an unsafe member path"
            )
    ...
    for member in zf.infolist():
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail="ZIP contains an invalid member"
            ) from exc
...
async def lora_upload_dataset(file: UploadFile = File(...)):
    ...
    if not dataset_name:
        raise HTTPException(
            status_code=400, detail="Invalid dataset name from filename"
        )
    ...
    try:
        ...
        if not os.path.exists(metadata_path):
            ...
            for entry in os.listdir(dataset_dir):
                ...
                if os.path.isdir(os.path.join(dataset_dir, entry)) and os.path.exists(
                    candidate
                ):
                    # Move contents up
                    ...
                    for item in os.listdir(nested):
                        shutil.move(
                            os.path.join(nested, item), os.path.join(dataset_dir, item)
                        )
                    os.rmdir(nested)
                    metadata_path = os.path.join(dataset_dir, "metadata.jsonl")
                    break
        ...
        if not os.path.exists(metadata_path):
            ...
            raise HTTPException(
                status_code=400, detail="ZIP must contain metadata.jsonl"
            )
...
async def lora_generate_dataset(
    request: LoraGenerateDatasetRequest, background_tasks: BackgroundTasks
):
    ...
    if request.samples:
        for s in request.samples:
            if s.text.strip():
                sample_list.append(
                    {"emotion": s.emotion.strip(), "text": s.text.strip()}
                )
    elif request.texts:
    ...
    if os.path.exists(dataset_dir):
        raise HTTPException(
            status_code=400, detail=f"Dataset '{safe_name}' already exists"
        )
    ...
    def task():
        ...
        try:
            ...
            for i, sample in enumerate(sample_list):
                ...
                description = (
                    f"{root_description}, {emotion}" if emotion else root_description
                )
                ...
                except Exception as e:  # noqa: BLE001 — per-sample generation error is recorded and processing continues
                    process_state["dataset_gen"]["logs"].append(f"  Failed: {e}")
        ...
        except Exception as e:  # noqa: BLE001 — dataset-gen failure cleans up partial output and reports
    ...
    if not _claim_process("dataset_gen"):
        raise HTTPException(
            status_code=400, detail="Dataset generation already running"
        )
...
async def lora_start_training(
    request: LoraTrainingRequest, background_tasks: BackgroundTasks
):
    ...
    if not os.path.isdir(dataset_dir):
        raise HTTPException(
            status_code=400, detail=f"Dataset '{request.dataset_id}' not found"
        )
    ...
    def on_training_complete():
        ...
        with lora_gpu_lock:
            ...
            if os.path.isdir(output_dir) and os.path.exists(
                os.path.join(output_dir, "training_meta.json")
            ):
                try:
                ...
                except Exception as e:  # noqa: BLE001 — best-effort manifest update in completion callback
...
async def lora_list_models():
    ...
    for m in models:
        ...
        m["preview_audio_url"] = (
            f"{url_prefix}/preview_sample.wav" if os.path.exists(preview_path) else None
        )
    return models
...
async def lora_delete_model(adapter_id: str):
    ...
    if any(m["id"] == adapter_id for m in builtin):
        raise HTTPException(
            status_code=403, detail="Built-in adapters cannot be deleted"
        )
    manifest = _load_manifest(LORA_MODELS_MANIFEST)
...
async def lora_download_builtin(adapter_id: str):
    ...
    entry = next(
        (e for e in manifest if e["id"] == hf_name or e["id"] == adapter_id), None
    )
    if not entry:
        raise HTTPException(
            status_code=404, detail=f"Unknown built-in adapter: {adapter_id}"
        )
    ...
    except Exception as e:  # noqa: BLE001 — download failures surface as HTTP 500
...
async def lora_test_model(request: LoraTestRequest):
    ...
    if not _try_acquire_lora_gpu():
        raise HTTPException(
            status_code=409,
            detail="LoRA training or another LoRA generation is using the GPU",
        )
    ...
    try:
        ...
        if not engine:
            raise HTTPException(
                status_code=500, detail="Failed to initialize TTS engine"
            )
        ...
        raise
    except Exception as e:  # noqa: BLE001 — TTS generation failures surface as HTTP 500
...
def _load_builder_state(name):
    ...
    if os.path.exists(state_path):
        try:
            ...
            return state
        except Exception:  # noqa: BLE001, S110 — stale/broken state file degrades to defaults
            pass
                ...
                projects.append(
                    {
                        ...
                        "done_count": sum(
                            1 for s in samples if s.get("status") == "done"
                        ),
                    }
                )
    return projects
...
async def dataset_builder_create(request: DatasetBuilderCreateRequest):
    ...
    if os.path.exists(work_dir):
        raise HTTPException(
            status_code=400, detail=f"Project '{safe_name}' already exists"
        )
    _save_builder_state(
        safe_name, {"description": "", "global_seed": "", "samples": []}
    )
...
async def dataset_builder_generate_sample(request: DatasetSampleGenRequest):
    ...
    with _get_builder_lock(safe_name):
        ...
        try:
            ...
            raise
        except Exception as e:  # noqa: BLE001 — per-sample generation error is recorded in builder state
...
async def dataset_builder_generate_batch(request: DatasetBatchGenRequest):
    ...
    def task():
        ...
        try:
            ...
            pstate["logs"].append(f"[DONE] Generated {completed}/{total} samples")
    ...
    with process_state_lock:
        ...
        project = projects.setdefault(
            safe_name, {"running": False, "logs": [], "cancel": False}
        )
...
async def dataset_builder_save(request: DatasetSaveRequest):
    ...
    try:
        ...
        if not os.path.exists(work_dir):
            raise HTTPException(
                status_code=404, detail="Dataset builder project not found"
            )
        ...
        done_samples = [
            (i, s) for i, s in enumerate(samples) if s.get("status") == "done"
        ]
        if not done_samples:
        ...
        if os.path.exists(dataset_dir):
            raise HTTPException(
                status_code=400,
                detail=f"Dataset '{safe_name}' already exists in training datasets",
            )
        ...
        async with aiofiles.open(
            os.path.join(dataset_dir, "ref_text.txt"), "w", encoding="utf-8"
        ) as f:
            await f.write(ref_text)

        # Write metadata
        async with aiofiles.open(
            os.path.join(dataset_dir, "metadata.jsonl"), "w", encoding="utf-8"
        ) as f:
            await f.write("\n".join(metadata_lines) + "\n")

        sample_count = len(metadata_lines)
        logger.info(
            f"Dataset saved: '{safe_name}' ({sample_count} samples, ref=sample_{ref_idx:03d})"
        )
        ...
        raise
    except Exception as e:  # noqa: BLE001 — save failure cleans up partial dataset and surfaces as HTTP 500
        # Clean up on failure
...
async def dataset_builder_delete(name: str):
    ...
    with process_state_lock:
        ...
        if project and project["running"]:
            raise HTTPException(
                status_code=409, detail="Dataset generation in progress"
            )
        with _get_builder_lock(safe_name):
            if not os.path.exists(work_dir):
                raise HTTPException(
...
async def preparer_start(
    ...
    try:
        ...
        if not has_space:
            raise HTTPException(
                status_code=400,
                detail=f"Insufficient disk space ({free_gb} GB free, 2 GB required).",
            )
    ...
    def _run():
        ...
        cmd = [
            ...
            audio_path,
            "--output",
            os.path.join(PREPARER_OUTPUT_DIR, config.output_filename),
            "--lang",
            config.lang,
            "--min-confidence",
...
async def preparer_batch_start(request: Request, background_tasks: BackgroundTasks):
    ...
    if not _claim_process("batch_preparer"):
        raise HTTPException(
            status_code=400, detail="Batch preparer is already running."
        )
    ...
    try:
        if request.headers.get("content-type", "").startswith("multipart/form-data"):
            ...
            try:
                tasks = [
                    BatchPreparerTask.model_validate(task)
                    for task in json.loads(form.get("tasks", "[]"))
                ]
                ...
                raise HTTPException(
                    status_code=422, detail=f"Invalid batch request: {e}"
                ) from e
            uploaded_files = form.getlist("audio_files")
            if len(uploaded_files) != len(tasks):
                raise HTTPException(
                    status_code=400,
                    detail="Each batch task must include an audio file.",
                )
            os.makedirs(UPLOADS_DIR, exist_ok=True)
            for task, uploaded_file in zip(tasks, uploaded_files):
                if not isinstance(uploaded_file, StarletteUploadFile):
                    raise HTTPException(
                        status_code=400, detail="Invalid audio file upload."
                    )
                if (
                    os.path.basename(uploaded_file.filename or "")
                    != task.audio_filename
                ):
                    raise HTTPException(
                        status_code=400,
                        detail="Uploaded filename does not match its task.",
                    )
                async with aiofiles.open(
                    os.path.join(UPLOADS_DIR, task.audio_filename), "wb"
                ) as destination:
                    await destination.write(await uploaded_file.read())
        else:
    ...
    if not has_space:
        ...
        raise HTTPException(
            status_code=400,
            detail=f"Insufficient disk space ({free_gb} GB free, 5 GB recommended).",
        )
    ...
    def _run():
        ...
        state["tasks"] = [
            {"audio": t.audio_filename, "status": "pending"}
            for t in batch_request.tasks
        ]
        ...
        try:
            for i, task in enumerate(batch_request.tasks):
                ...
                if not os.path.exists(audio_path):
                    state["logs"].append(
                        f"[{i + 1}/{len(batch_request.tasks)}] Skipping — audio not found: {task.audio_filename}"
                    )
                    state["tasks"][i]["status"] = "failed"
                    continue

                state["logs"].append(
                    f"--- [{i + 1}/{len(batch_request.tasks)}] {task.audio_filename} ---"
                )
                ...
                cmd = [
                    ...
                    audio_path,
                    "--output",
                    os.path.join(PREPARER_OUTPUT_DIR, task.output_filename),
                    "--lang",
                    batch_request.lang,
                    "--min-confidence",
                    str(batch_request.min_confidence),
                    "--min-snr",
                    str(batch_request.min_snr),
                ]

                rc = _stream_subprocess_to_logs(
                ...
                if state.get("cancel"):
                    ...
                    state["tasks"][i]["status"] = "failed"
                    state["logs"].append(
                        f"[{i + 1}] Failed (exit {rc}): {task.audio_filename}"
                    )
        except Exception as e:
