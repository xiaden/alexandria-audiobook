...
| `gc_expired_artifacts` | `gc_expired_artifacts(render_root, *, job_retention_days, chunk_retention_days) -> dict` | Tombstoning GC sweep: deletes run dir files for completed jobs older than effective retention (longer of job/chunk), restricted to paths resolved as strict children of `render_root`; tombstones render_chunk→'evicted' and render_job→'expired' in the same sweep; snapshot-reference eligibility union keeps referenced run dirs alive; rows never time-deleted. (Plan C) |
...
| Name | Notes |
|------|-------|
| `ConcurrentTransactionError(RuntimeError)` | Raised when a write is attempted from a thread that does not own the open transaction, or when `BEGIN IMMEDIATE` times out under contention. Mapped to 503 + `Retry-After: 5` in the api layer — **implemented in Plan K** via `app.add_exception_handler(ConcurrentTransactionError, concurrent_transaction_error_handler)` in app.py (module-level handler; JSON 503 body `{"detail": "Concurrent write in progress — retry after the advertised delay"}`, `Retry-After: 5` via `_CONCURRENT_WRITE_RETRY_AFTER_SECONDS = 5` matching the 409 snapshot convention). App-wide: CTE only originates from pipeline storage writes. 409 + Retry-After:5 snapshot semantics (`POST /projects/load`) unchanged. Walk-side `_retry_write` (runner.py) backoff-retry unchanged. |
...
### tts_integration (Plans B/C; Plan J: pause passthrough + single-speaker enforcement)
| Method | Notes |
|--------|-------|
| `render_audiobook` (extended) | Now takes `job_id`; writes render_job row (running) + per-chunk render_chunk rows in individual mode; final transaction sets completed + output_artifact_path. Rows = truth. (Plan J) Now also takes `tts_config: dict | None` — the `tts` section of config.json, resolved by api_export render() via `load_tts_config` at request time; its `pause_between_speakers_ms`/`pause_same_speaker_ms` ride through the boundary onto batch chunk dicts for a merge step. `_enforce_single_speaker` applied at step 4b (after empty-script early return, before chunk dispatch). |
| `_enforce_single_speaker` | `_enforce_single_speaker(voice_config, storage, book_id) -> dict` — render-boundary-only normalization (decision #9): parameterized read of `book.single_speaker`; when set, ensures a NARRATOR entry (covers no-NARRATOR-span scripts) and replaces every entry with a copy of the resolved narrator config; 0/missing row → returned unchanged; one dict covers batch + individual dispatch. (Plan J) |
| `_resolve_pause_ms` | `_resolve_pause_ms(tts_config: dict | None) -> tuple[int, int]` — per-field fallback to `PAUSE_BETWEEN_SPEAKERS_MS`/`PAUSE_SAME_SPEAKER_MS` (500/250) so a partial config keeps the untouched field's default; `None` → both defaults. (Plan J) |
| `PAUSE_BETWEEN_SPEAKERS_MS` / `PAUSE_SAME_SPEAKER_MS` | Module constants 500 / 250 (mirror the TTSConfig pydantic defaults); used by `_resolve_pause_ms` and as `_build_chunks` keyword defaults. (Plan J) |
| `_build_chunks` | `_build_chunks(script, *, pause_between_speakers_ms=500, pause_same_speaker_ms=250) -> list[dict]` — new keyword-only pause kwargs (backward compatible; existing callers unchanged); both values carried on EVERY chunk so the configured global pauses survive the render boundary (engine ignores unknown chunk keys); per-span `pause_after` NOT restored (cannot-restore #5). (Plan J) |
...
| GET `/api/pipeline/review/{book_id}` | api_review | Union queue with `walkitem:` prefixed ids. (Plan J) Every item is enriched with `neighbors: {"before": [...], "after": [...]}` — up to 2 span texts in presentation order around the item's span reference (`character_span` junction → `related_entity_id`; walk item kind `instruction` → `target_id`); empty lists for items without a span reference (character_book/character_scene/voice_profile/voice_assignment) and for first/last-span edge cases. Enrichment lives in review.py get_review_items; api_review.py returns manager output verbatim. |
...
### New (Plan B: runs/jobs/chunks list; Plan C: audio/chunk/export; Plan I: projects; Plan J: single-speaker flag)
...
| POST `/api/pipeline/export/m4b` | api_export | C — 3-phase FFMETADATA1 export. (Plan K) Success response extended with `pauses_applied: False` and `pauses_message` — explicit capability disclosure: global pause values are carried per-chunk but the engine does not insert audible silence into merged audio (`app/tts.py` immutable; per-span `pause_after` not restorable — cannot-restore #5). |
| GET `/api/pipeline/export/mp3/{job_id}` | api_export | K — row-backed MP3 artifact serving: `FileResponse404` + `_media_type_for_artifact` (`audio/mpeg`), filename `audiobook.mp3`; dispatch mirrors download_render/export_m4b (unknown 404, expired 410, non-completed 404, run dir missing 404, artifact missing JSON 404); path from hardcoded `os.path.join(run_dir, "audiobook.mp3")` constant — no user input; no legacy names |
| GET `/api/pipeline/export/audacity/{job_id}` | api_export | K — row-backed Audacity zip serving: `FileResponse404` + `_media_type_for_artifact` (`application/zip`), filename `audiobook-audacity.zip`; same dispatch + hardcoded-constant path as mp3 route |
...
| GET `/api/pipeline/book/{book_id}/single_speaker` | api_operations | J — read flag: 200 `{book_id, single_speaker: 0|1}`, 404 unknown book (parameterized SELECT; unset reads back 0) |
| PUT `/api/pipeline/book/{book_id}/single_speaker` | api_operations | J — write flag: body `{single_speaker: bool}` via pydantic SingleSpeakerUpdateRequest → 200 `{status:'ok', book_id, single_speaker: 0|1}`, 404 unknown book, 422 invalid body (parameterized UPDATE; value normalized 0/1) |
...
| ReviewItem (extended) | D | item_id (junction:/walkitem: prefixed), kind, target_table, target_id, prior_value, created_ms; confidence/human_override are junction-only fields — walk items carry exactly {item_id, kind, target_table, target_id, prior_value, created_ms}. (Plan J) Every item also carries `neighbors: {"before": [...], "after": [...]}` — up to 2 span texts in presentation order around the item's span reference; empty lists for items without a span reference and for first/last-span edges. Frontend type marks the field optional (`neighbors?`). |
...
| SingleSpeakerUpdateRequest | J | single_speaker: bool — request body for PUT /api/pipeline/book/{book_id}/single_speaker (missing/invalid body → 422) |
...
**Plan L (Pause Insertion Tuning) IMPLEMENTED + QA-VALIDATED 2026-08-11** — commit 331311a; book-tier pause overrides reach render assembly: `render_audiobook` Step 4c resolved render-time via a parameterized, storage-backed read of `book.pause_between_speakers_ms`/`pause_same_speaker_ms` (NULL = fall-through, 0 = intentional no-gap), threaded through batch + individual render paths into `_assemble_paused_artifact` with request → book → config → 500/250 precedence via `resolve_effective_pauses`; e2e proof decodes real WAV chunks asserting book-tier silence + canonical artifact publication; truthful tri-state (`applied`/`failed`) on export responses, `pending` (conservative) on render_status; stale comments reconciled across api_export.py / tts_integration.py / editor-pipeline.ts + READMEs + CONTRACTS + Phase 2/4/5 concrete subsections; QA Round 2 PASS (test + docs analyzers); 2 known pre-existing env-only failures (both fail at HEAD pre-Plan-L): `test_render_503_when_production_engine_is_none` (gradio_client) and `test_render_route_returns_503_without_engine`; app/tts.py byte-for-byte untouched.
